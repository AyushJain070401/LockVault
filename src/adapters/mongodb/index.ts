...
interface MongoCollection {
  ...
  updateOne(filter: unknown, update: unknown, options?: Record<string, unknown>): Promise<{ modifiedCount: number; upsertedCount: number }>;
...
export function createMongoDBAdapter(db: MongoDb, options: { collectionPrefix?: string } = {}): DatabaseAdapter {
  ...
  return {
    ...
    async close() { /* managed externally */ },
    ...
    async storeTOTPSecret(userId, secret) {
      await col('totp_secrets').updateOne({ _id: userId as unknown }, { $set: { secret } }, { upsert: true });
    },
