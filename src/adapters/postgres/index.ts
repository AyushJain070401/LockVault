...
export function createPostgresAdapter(pool: PgPool, options: { tablePrefix?: string } = {}): DatabaseAdapter {
  ...
  async initialize(): Promise<void> {
    await pool.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_${tablePrefix}sessions_user ON ${t('sessions')} (user_id);
      ...
      CREATE INDEX IF NOT EXISTS idx_${tablePrefix}oauth_provider ON ${t('oauth_links')} (provider, provider_user_id);
    `);
  }
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    const { rowCount } = await pool.query(
      `DELETE FROM ${t('sessions')} WHERE id = $1`, [sessionId],
    );
    return (rowCount ?? 0) > 0;
  }
  ...
  async deleteSessionsByUser(userId: string): Promise<number> {
    const { rowCount } = await pool.query(
      `DELETE FROM ${t('sessions')} WHERE user_id = $1`, [userId],
    );
    return rowCount ?? 0;
  }
  ...
  async deleteExpiredSessions(): Promise<number> {
    const { rowCount } = await pool.query(
      `DELETE FROM ${t('sessions')} WHERE expires_at < NOW() OR is_revoked = TRUE`,
    );
    return rowCount ?? 0;
  }
  ...
  async revokeRefreshTokenFamily(family: string): Promise<void> {
    await pool.query(
      `UPDATE ${t('refresh_families')} SET revoked = TRUE WHERE family = $1`, [family],
    );
  }
  ...
  async isRevoked(jti: string): Promise<boolean> {
    const { rows } = await pool.query(
      `SELECT 1 FROM ${t('revocation_list')} WHERE jti = $1`, [jti],
    );
  ...
  async cleanupRevocationList(): Promise<number> {
    const { rowCount } = await pool.query(
      `DELETE FROM ${t('revocation_list')} WHERE expires_at < NOW()`,
    );
    return rowCount ?? 0;
  }
  ...
  async getTOTPSecret(userId: string): Promise<string | null> {
    const { rows } = await pool.query(
      `SELECT secret FROM ${t('totp_secrets')} WHERE user_id = $1`, [userId],
    );
  ...
  async getBackupCodes(userId: string): Promise<string[]> {
    const { rows } = await pool.query(
      `SELECT code FROM ${t('backup_codes')} WHERE user_id = $1`, [userId],
    );
  ...
  async getOAuthLinks(userId: string): Promise<OAuthLink[]> {
    const { rows } = await pool.query(
      `SELECT * FROM ${t('oauth_links')} WHERE user_id = $1`, [userId],
    );
