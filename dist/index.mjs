...
var JWTManager = class _JWTManager {
  ...
  sign(payload, secret) {
    ...
    switch (algorithm) {
      ...
      case "EdDSA": {
        const edSig = sign(null, inputBuf, this.config.jwt.privateKey);
  ...
  verifySignature(algorithm, signingInput, signature, secret) {
    ...
    switch (algorithm) {
      ...
      case "EdDSA":
        return verify(null, inputBuf, this.config.jwt.publicKey, sigBuf);
      default:
        return false;
    }
  }
...
export { AuthErrorCode, ConfigurationError, JWTManager, LockVault, LockVaultError, MemoryAdapter, MemoryKeyValueStore, OAuthError, OAuthManager, RateLimitError, RateLimiter, RefreshTokenReuseError, SessionError, SessionManager, TOTPError, TOTPManager, TokenExpiredError, TokenInvalidError, TokenRevokedError, generateBackupCodes, generateId, generateUUID, hashPassword, verifyPassword };
