import { randomBytes, scrypt, createHash, sign, createSign, createHmac, verify, createVerify, timingSafeEqual, createCipheriv, createDecipheriv } from 'crypto';
...
function createJWTManager(config, hooks = {}) {
  ...
  function signToken(payload, secret) {
    ...
    switch (algorithm) {
      ...
      case "ES512": {
        const s = createSign("SHA512");
        s.update(signingInput);
        signature = s.sign({ key: config.jwt.privateKey, dsaEncoding: "ieee-p1363" }, "base64url");
        break;
      }
      case "EdDSA": {
        const edSig = sign(null, inputBuf, config.jwt.privateKey);
  ...
  function verifySignature(algorithm, signingInput, signature, secret, kid) {
    ...
    switch (algorithm) {
      ...
      case "ES512": {
        const v = createVerify("SHA512");
        v.update(signingInput);
        return v.verify({ key: config.jwt.publicKey, dsaEncoding: "ieee-p1363" }, sigBuf);
      }
      case "EdDSA":
        return verify(null, inputBuf, config.jwt.publicKey, sigBuf);
      default:
        return false;
    }
  }
...
export { AuthErrorCode, ConfigurationError, EmailError, LockVaultError, OAuthError, RateLimitError, RefreshTokenReuseError, SessionError, TOTPError, TokenExpiredError, TokenInvalidError, TokenRevokedError, createJWTManager, createLockVault, createMemoryAdapter, createMemoryKeyValueStore, createOAuthManager, createRateLimiter, createSessionManager, createTOTPManager, generateBackupCodes, generateCSRFToken, generateId, generatePKCE, generateTokenFingerprint, generateUUID, hashPassword, sanitizeIpAddress, verifyPassword };
