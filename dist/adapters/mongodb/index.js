...
var MongoDBAdapter = class {
  ...
  async createSession(session) {
    await this.col("sessions").insertOne({ _id: session.id, ...session });
    return session;
  }
  async getSession(sessionId) {
    const doc = await this.col("sessions").findOne({ _id: sessionId });
  ...
  async updateSession(sessionId, updates) {
    ...
    return result ? this.mapDoc(result) : null;
  }
  ...
  async getRefreshTokenFamily(family) {
    const doc = await this.col("refresh_families").findOne({ _id: family });
    if (!doc) return null;
  ...
  async isRevoked(jti) {
    const doc = await this.col("revocation_list").findOne({ _id: jti });
    return doc !== null;
  }
  async cleanupRevocationList() {
    const result = await this.col("revocation_list").deleteMany({ expiresAt: { $lt: /* @__PURE__ */ new Date() } });
    return result.deletedCount;
  }
  // ─── TOTP ──────────────────────────────────────────────────────────────
  ...
  async getTOTPSecret(userId) {
    const doc = await this.col("totp_secrets").findOne({ _id: userId });
    return doc?.secret ?? null;
  }
  async removeTOTPSecret(userId) {
    await this.col("totp_secrets").deleteOne({ _id: userId });
